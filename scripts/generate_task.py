...
def main(api_key):
    ...
    prompt = (f"Create a new programming task based on this template: {template}. "
              ...
              "Use the following existing code and tests as inspiration. Also use the structure of the original task description based on the .md fild to make sure that the new generated tasks look aesthetically pleasing and is detailed about the task and what the studnet has to do and also gives an interesting description based on the theme they have chosen. The new task should include specific function names where necessary."
              "Also, provide a set of tests for the task and a suggested solution. The tests have to be thorough as they will be used to evaluate the solution for every student. It is therefore extremly important that the tests are correct and cover all edge cases. Also as the tests can be hidden in certain tasks it is important that the students recieve the name of the test class and the test method for their functions in the task description. "
              "The code file that is generated must include a proper code template that is compatible with the tests that are generated so that the studnets use the right function names and the right return types, to ensure that the tests pass."
